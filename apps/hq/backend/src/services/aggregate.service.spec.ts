...
describe("getAggregate — fan out and merge", () => {
  ...
  it("de-collides ids across countries with a per-country offset", async () => {
    ...
    // Warehouses de-collide the same way.
    expect(new Set(data.warehouses.map((w) => w.id)).size).toBe(3);
...
describe("getAggregate — warehouses", () => {
  it("builds warehouses enriched with their country, source and lot count", async () => {
...
describe("enrichWarehouses", () => {
  it("attaches the latest reading and flags out-of-range", async () => {
    ...
    const { getAggregate, enrichWarehouses } = await loadService(clients);
    ...
    // Routed to the owning country's client with the *local* warehouse id.
    expect(brazil.fetchWarehouseMeasures).toHaveBeenCalledWith(1);
  });
